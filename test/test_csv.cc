...
TEST_F(ReadCsvValid, AllFieldTypes) {
  EXPECT_EQ(
      read_csv(&r, csv, "./csv_valid/valid_rows_with_all_field_types.csv"), 3);
  EXPECT_EQ(csv->rows[0].size, 7);
...
TEST_F(ReadCsvValid, SpecialCharacters) {
  EXPECT_EQ(read_csv(&r, csv, "./csv_valid/valid_special_characters.csv"), 3);
  EXPECT_EQ(csv->rows[0].size, 1);
  EXPECT_EQ(csv->rows[1].size, 1);
  EXPECT_EQ(csv->rows[2].size, 1);
}

TEST_F(ReadCsvValid, SymbolsAndPunctuation) {
  EXPECT_EQ(read_csv(&r, csv, "./csv_valid/valid_symbols_and_punctuation.csv"),
            3);
  EXPECT_EQ(csv->rows[0].size, 1);
  EXPECT_EQ(csv->rows[1].size, 1);
  EXPECT_EQ(csv->rows[2].size, 1);
...
TEST_F(ReadCsvValid, UnicodeText) {
  EXPECT_EQ(read_csv(&r, csv, "./csv_valid/valid_unicode_text.csv"), 4);
  EXPECT_EQ(csv->rows[0].size, 2);
  EXPECT_EQ(csv->rows[1].size, 2);
  EXPECT_EQ(csv->rows[2].size, 2);
  EXPECT_EQ(csv->rows[3].size, 2);
...
TEST_F(ReadCsvValid, UniformValues) {
  EXPECT_EQ(read_csv(&r, csv, "./csv_valid/valid_uniform_values.csv"), 4);
  EXPECT_EQ(csv->rows[0].size, 3);
  EXPECT_EQ(csv->rows[1].size, 3);
  EXPECT_EQ(csv->rows[2].size, 3);
  EXPECT_EQ(csv->rows[3].size, 3);
